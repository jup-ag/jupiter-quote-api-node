/* tslint:disable */
/* eslint-disable */
/**
 * Jupiter API v6
 * The core of [jup.ag](https://jup.ag). Easily get a quote and swap through Jupiter API.  ### Rate Limit We update our rate limit from time to time depending on the load of our servers. We recommend running your own instance of the API if you want to have high rate limit, here to learn how to run the [self-hosted API](https://station.jup.ag/docs/apis/self-hosted).  ### API Wrapper - Typescript [@jup-ag/api](https://github.com/jup-ag/jupiter-quote-api-node)  ### Data types - Public keys are base58 encoded strings - raw data such as Vec<u8\\> are base64 encoded strings 
 *
 * The version of the OpenAPI document: 6.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface RfqSwapRequest
 */
export interface RfqSwapRequest {
    /**
     * 
     * @type {string}
     * @memberof RfqSwapRequest
     */
    requestId: string;
    /**
     * 
     * @type {string}
     * @memberof RfqSwapRequest
     */
    quoteId: string;
    /**
     * Base64 encoded versioned transaction
     * @type {string}
     * @memberof RfqSwapRequest
     */
    transaction: string;
}

/**
 * Check if a given object implements the RfqSwapRequest interface.
 */
export function instanceOfRfqSwapRequest(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "requestId" in value;
    isInstance = isInstance && "quoteId" in value;
    isInstance = isInstance && "transaction" in value;

    return isInstance;
}

export function RfqSwapRequestFromJSON(json: any): RfqSwapRequest {
    return RfqSwapRequestFromJSONTyped(json, false);
}

export function RfqSwapRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): RfqSwapRequest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'requestId': json['requestId'],
        'quoteId': json['quoteId'],
        'transaction': json['transaction'],
    };
}

export function RfqSwapRequestToJSON(value?: RfqSwapRequest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'requestId': value.requestId,
        'quoteId': value.quoteId,
        'transaction': value.transaction,
    };
}

